...
router.post("/query", async (req, res) => {
  try {
    ...
    //pasarla el cacheManager al query builder, antes no se pasaba por eso no funcionaba el obtener resultados
    const cacheManager = req.app.locals.cacheManager;
    const queryBuilder = new QueryBuilder(connector, cacheManager);
...
router.post("/insert", async (req, res) => {
  try {
    ...
    //invalidar el cache, si no se invalida se siguen teniendo los datos veijos
    const cacheManager = req.app.locals.cacheManager;
    if (cacheManager) cacheManager.invalidate(table);
...
router.get("/cache/stats", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error obteniendo estadísticas", message: error.message });
  }
});
...
router.post("/cache/clear", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error limpiando cache", message: error.message });
  }
});
